...
const Auth = () => {
  ...
  return (
    <div className='flex items-center justify-center flex-wrap'>
      <h1 className='font-bold text-3xl w-full'>CÂU 5 - Auth <br /></h1>
      ...
      {!isLoggedIn ? (
        <div className="flex gap-2 ">
          ...
          />
          <button className="bg-green-300 hover:bg-green-500 px-2 text-black rounded" onClick={handleLogin}>
            Đăng nhập
          </button>
        </div>
      ) : (
        <div>
          ...
          <button onClick={handleLogout} className="bg-red-300 hover:bg-red-500 text-black px-2 rounded">Đăng xuất</button>
        </div>
      )}
    </div>
  );
